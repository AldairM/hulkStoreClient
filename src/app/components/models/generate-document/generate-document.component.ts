...
export class GenerateDocumentComponent implements OnInit {
      ...
      this._snackBar.openFromComponent(SnackAlertComponent,
        {
          data: {
            type: 'error',
            message: 'La fila que tratas de agregar no es valida'
          },
          panelClass: 'error'
        });
    }
            ...
            this._snackBar.openFromComponent(SnackAlertComponent,
              {
                data: {
                  type: 'success',
                  message: `documento # ${response.consecutive}`
                },
                panelClass: 'success'
              });
        ...
        this._snackBar.openFromComponent(SnackAlertComponent,
          {
            data: {
              type: 'error',
              message: 'Debes completar todo el documento'
            },
            panelClass: 'error'
          });
      }
  ...
  validateDocument(): boolean {
    if (this.commercialDocument.customer == undefined || this.commercialDocument.customer == null || this.commercialDocument.customer.id == 0) {
      return false;
    }

    if (this.commercialDocument.typeDocument == undefined || this.commercialDocument.typeDocument == null || this.commercialDocument.typeDocument.id == 0) {
      return false;
    }
